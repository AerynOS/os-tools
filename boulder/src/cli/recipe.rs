...
fn parse_upd_source(s: &str) -> Result<UpdSource, String> {
    match s.strip_prefix(stone_recipe::upstream::GIT_PREFIX) {
        Some(git_ref) => Ok(UpdSource::Git(git_ref.to_owned())),
        None => Ok(UpdSource::Plain(s.parse::<Url>().map_err(|e| e.to_string())?)),
    }
}
...
fn update(
    ...
    for (i, (original, update)) in parsed.upstreams.into_iter().zip(sources).enumerate() {
        match (original.props, update) {
            ...
            (Props::Git { .. }, UpdSource::Git(new_ref)) => {
                let key = value["upstreams"][i]
                    .as_mapping()
                    .and_then(|map| map.keys().next())
                    .cloned();
