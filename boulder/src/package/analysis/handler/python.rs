...
pub fn python(bucket: &mut BucketMut<'_>, info: &mut PathInfo) -> Result<Response, BoxError> {
    ...
    for dep in deps.lines() {
        bucket.dependencies.insert(Dependency {
            kind: dependency::Kind::Python,
            name: format!("{}", pep_503_normalize(&dep.to_string())?),
        });
    }
...
fn pep_503_normalize(input: &String) -> Result<String, BoxError>  {
...
mod test {
    ...
    fn test_normalization() {
        assert_eq!(pep_503_normalize(&"PyThOn-_-foo".to_string()).unwrap(), "python-foo");
        assert_eq!(pep_503_normalize(&"PyThOn.-f-oo".to_string()).unwrap(), "python-f-oo");
    }
}
