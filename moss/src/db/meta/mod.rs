...
mod test {
    ...
    fn db_queries_by_provider() -> Result<(), Error> {
        ...
        let mut entries = create_db()?.query(Some(Filter::Provider(common_provider())))?;
    ...
    fn db_queries_by_dependency() -> Result<(), Error> {
        ...
        let mut entries = create_db()?.query(Some(Filter::Dependency(dependency)))?;
        entries.sort_by(|(id1, _), (id2, _)| id1.cmp(&id2));
    ...
    fn db_queries_by_name() -> Result<(), Error> {
        ...
        let meta = create_db()?.query(Some(Filter::Name(package::Name::from(format!(
            "Name {}",
            NUM_ENTRIES / 2
        )))))?;
    ...
    fn db_queries_by_keyword() -> Result<(), Error> {
        {
            ...
            let expected_entries = all_entries()
                ...
                .sorted_by(|(id1, _), (id2, _)| id1.cmp(&id2));
            let mut entries = create_db()?.query(Some(Filter::Keyword("Summary")))?;
            entries.sort_by(|(id1, _), (id2, _)| id1.cmp(&id2));
            itertools::assert_equal(entries.into_iter(), expected_entries);
        }
        {
            ...
            let expected_entries = all_entries()
                ...
                .sorted_by(|(id1, _), (id2, _)| id1.cmp(&id2));
            let mut entries = create_db()?.query(Some(Filter::Keyword("Name 10")))?;
            entries.sort_by(|(id1, _), (id2, _)| id1.cmp(&id2));
            itertools::assert_equal(entries.into_iter(), expected_entries);
        }
    ...
    fn db_queries_all() -> Result<(), Error> {
        let expected_entries = all_entries()
            ...
            .sorted_by(|(id1, _), (id2, _)| id1.cmp(id2));
        let mut entries = create_db()?.query(None)?;
        entries.sort_by(|(id1, _), (id2, _)| id1.cmp(id2));
        itertools::assert_equal(entries.into_iter(), expected_entries);
        Ok(())
    }
