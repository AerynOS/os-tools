...
impl Database {
    ...
    pub fn add(
        ...
        let id = self.conn.exec_mut(|conn| -> Result<i64, rusqlite::Error> {
            ...
            {
                let mut selection_stmt = tx.prepare(
    ...
    pub fn batch_remove<'a>(&mut self, ids: impl IntoIterator<Item = &'a Id>) -> Result<(), Error> {
        self.conn.exec_mut(|conn| {
            let tx = conn.transaction()?;
            {
                let mut stmt = tx.prepare("DELETE FROM state WHERE id = ?")?;
                for id in ids {
